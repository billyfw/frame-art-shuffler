# Frame Art Shuffler – Integration Roadmap & Status# Frame Art Shuffler Integration Roadmap
...
### Phase 1 – HACS Foundation (✅ October 2025)    "Madrone": {
...
### Phase 1.5 – HA-Native Storage & Cleanup (✅ November 2025)- `instance_id` is a UUID generated by the integration (persisted in the config entry). `friendly_name` mirrors the home string for clarity and can hold a user-facing label later.

- Removed git persistence (`git_sync.py`) and the try-commit-or-revert UX.- Claim rules:

- Removed add-on communication modules (`notify.py`, `addon_api.py`) and the “Sync to Add-on” button.  1. During config flow, read `homes`. If `home` is absent, write `{instance_id, friendly_name}`.

- Text/number entities now read/write config entry data only.  2. If `home` exists with a different `instance_id`, abort with an error (“Home already claimed by another integration instance”).
...
### Phase 2 – Built-in Shuffle Scheduler (🔄 next)## Phase 1 – HACS-Ready Integration Foundation

Goal: the integration handles shuffle timing without user-created automations.**Status:** ✅ Completed (October 2025)
...
### Phase 3 – Observability & Diagnostics (📝 planned)2. **Config flow & reauth**
...
### Phase 4 – Control Surface Expansion (🎯 future)

- Add power/art mode control via `MediaPlayerEntity` or a bespoke platform.3. **Metadata helper module**

- Additional services: brightness, “shuffle with filters”, metadata refresh, etc.   - Read/write `metadata.json` atomically (temp file + rename) under a shared lock.
...
### Phase 5 – Automation & Analytics (🚀 longer term)
...
- Optional structured shuffle log (`/config/frame_art/shuffle_log.json`).   - No control services yet—focus on state reflection and metadata persistence.
...
| **Logger** | Enable `custom_components.frame_art_shuffler: debug` to trace scheduler events and shuffle results. | View via *Settings → System → Logs* or tail `home-assistant.log`. |   - Unit tests for metadata helper (home claim collisions, TV upsert, tag normalization).
...
- Git sync workflow and persistent notifications tied to Git.## Phase 2 – Add-on Alignment (external work)
...
- “Add-on sync failure” persistent notifications.

**Status:** ⚠️ ARCHITECTURE CHANGED - See [ARCHITECTURE_REFACTOR.md](ARCHITECTURE_REFACTOR.md)
...
- No integration → add-on communication required now.Required changes:
...
- “Release home claim” maintenance tool for metadata cleanup.   - **Body:**
...
Use this roadmap as the authoritative status tracker. Update the status badges (✅, 🔄, 📝, 🎯, 🚀) as milestones land.         "shuffleFrequencyMinutes": 30
